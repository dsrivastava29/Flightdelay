...
public class AverageFlightDelayByYear {
	public static class AverageFlightDelayByYearMapper extends Mapper<Object, Text, AirlineTextPair, Text> {
		...
		public void map(Object offset, Text value, Context context) throws IOException, InterruptedException {
			...
			// Parse the input line
						...
						if (line.length > 0 && isValidEntry(line)) {
							...
							key.setAirLineName(line[8].trim());
							key.setYear(line[0].trim());
	 ...
	 * Reduce class
	 */
	public static class AverageFlightDelayByYearReducer extends Reducer<AirlineTextPair, Text, AirlineTextPair, IntWritable> {
	...
	/**
	 * GroupComparator ignores the month files of the key so that single reduce
	 * function call receives all flight data for given unique carrier name
	 */
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(AverageFlightDelayByYear.class);
		job.setMapperClass(AverageFlightDelayByYearMapper.class);
		job.setReducerClass(AverageFlightDelayByYearReducer.class);
